...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		vertex1 = Cross(screenVertices[0], screenVertices[1]);
		vertex2 = Cross(screenVertices[1], screenVertices[2]);
		...
		if (Dot(cameraPosition, Cross(vertex1, vertex2)) <= 0) {
			Novice::DrawTriangle(
		...
		Novice::ScreenPrintf(0, 40, "%f : Dot", Dot(cameraPosition, Cross(vertex1, vertex2)));
		///
		/// ↑描画処理ここまで
		///
